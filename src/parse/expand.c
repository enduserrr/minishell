...
static ssize_t next_env(char *str, size_t env, char is_heredoc)
{
    ...
    backslash = 0;
    if (str[env + 1])/*This was while() loop but didn't stop correctly. Fix it*/
    {
        if (str[env] == '$' && !backslash && (is_heredoc || quote != '\''))
        {
            if (str[env + 1] == '?')
				return (2);
            if (isalnum(str[env + 1]) || strchr("!#$&*-?@_", str[env + 1]) ||
                (str[env + 1] == '\\' && str[env + 2] != '\\') ||
                (!is_heredoc && !quote && strchr("\'\"", str[env + 1])))
                return env;
        }
        if (!quote && strchr("\'\"", str[env]))
            quote = str[env];
...
char	ft_expand(char **s, int status, char id)
{
	...
	{
		// len = env_len(*s + env);
		val = check_env(*s + env + 1, env_len(*s + env) - 1, status);
		if (!val && env == 0 && env_len(*s + env) == ft_strlen(*s))
		{
			if (id == IN_FILE || id == OUT_FILE || id == OUT_A_FILE)
				return (set_err(AMBIG_REDIR_ERR, *s), RETURN_FAILURE);
			return (RMV);
		}
		printf("%s %s %zu %zu\n", *s, val, env, env_len(*s + env));
		if (ft_strinsrt(s, val, env, env_len(*s + env)))
			env += ft_strplen(val);
		free(val);
